...
Irreversible actions are kept to a minimum. Reversible computers can still do all the same things irreversible computers can, but tend to produce "garbage" — data that is a by-product from computing a result. However, the garbage can be kept and then used to *uncompute* the value after it's been safely copied, producing no waste. This allows expressions to be possible despite being irreversible sometimes.
